pipeline {
    ...
    stages {
        stage("select node and run in docker") {
            ...
            steps {
                script {
                    def hostHostname = sh(script: 'hostname', returnStdout: true).trim()
                    docker.image('cuda:1.0').inside("--user  root -e HOST_HOSTNAME=${hostHostname} -v /etc/localtime:/etc/localtime:ro --privileged --gpus all") {
                        // 提取到外面统一处理步骤
                        checkout([
                            $class: 'GitSCM',
